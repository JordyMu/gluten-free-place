import pastaRegaloPhoto from "@/assets/pasta-regalo-beetroot-fettuccine.webp";
import sirEatALotPhoto from "@/assets/sir-eat-a-lot-gf-bread.jpg";
import offGlutenPathPizzaBases from "@/assets/off-gluten-path-pizza-bases.jpg";

export interface Restaurant {
  name: string;
  slug: string;
  address: string;
  city: string;
  country: string;
  hours: string;
  phone: string;
  website: string;
  directionsUrl: string;
  specialty: string;
  overview: string;
  menuHighlights: string[];
  proTip: string;
  icon: string;
  featured: boolean;
  cuisineTypes: string[];
  celiacSafe: "dedicated-facility" | "protocols-in-place";
  menuType: "fully-gluten-free" | "mixed-menu";
  rating: number;
  reviewCount: number;
  lat: number;
  lng: number;
  venueType: "bakery" | "restaurant" | "cafe" | "supermarket" | "street-food" | "home-baker" | "gf-products";
  photos: (string | { url: string; caption?: string })[];
  menuNotes?: string[];
  menuPhotos?: string[];
  staffKnowledgeScore?: number;
  celiacSafetyScore?: number; // 1-10 score for "Is this place safe for celiac?"
  whyPeopleLoveIt?: string[];
  nearby?: { label: string; name: string }[];
}

export const capeTownRestaurants: Restaurant[] = [
  {
    name: "Off the Gluten Path - Sea Point",
    slug: "off-the-gluten-path-sea-point",
    address: "277 Main Rd, Sea Point, Cape Town, 8060",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Sat: 8:00AM – 5:00PM",
    phone: "+27 21 434 0015",
    website: "www.offtheglutenpath.co.za",
    directionsUrl: "https://www.google.com/maps/search/Off+the+Gluten+Path+277+Main+Rd+Sea+Point+Cape+Town",
    specialty: "100% Gluten-Free Bakery & Café",
    overview: "Off the Gluten Path is Cape Town's premier dedicated gluten-free bakery and café. Everything on the menu is completely gluten-free, making it a safe haven for celiacs. The bakery uses a dedicated facility with zero cross-contamination risk, and all ingredients are carefully sourced to ensure safety for those with celiac disease.",
    menuHighlights: ["🥐 Fresh Croissants & Pastries (GF)", "🥖 Artisan Breads (GF)", "🍰 Cakes & Desserts (GF)", "🥗 Light Meals & Sandwiches (GF)"],
    proTip: "Their croissants are legendary - get there early!",
    icon: "🥐",
    featured: true,
    cuisineTypes: ["Bakery", "Café", "Gluten-Free"],
    celiacSafe: "dedicated-facility",
    menuType: "fully-gluten-free",
    rating: 4.9,
    reviewCount: 245,
    lat: -33.9175,
    lng: 18.3856,
    venueType: "bakery",
    photos: [
      { url: offGlutenPathPizzaBases, caption: "Pizza Bases (2 Pack) — Ingredients: Brown Rice Flour, Tapioca Starch, GF Oat Flour, Castor Sugar, Psyllium Husk, Water, Yeast, Salt, Olive Oil, Apple Cider Vinegar" },
      "https://images.unsplash.com/photo-1509440159596-0249088772ff?w=800",
      "https://images.unsplash.com/photo-1555507036-ab1f4038808a?w=800",
      "https://images.unsplash.com/photo-1517433670267-08bbd4be890f?w=800"
    ],
    menuNotes: [
      "All products made in a dedicated gluten-free facility",
      "Uses certified gluten-free flour blends",
      "Fresh bakes available daily from 8am",
      "Pre-orders recommended for special cakes",
      "Pizza Bases available in 2-packs — see photo for full ingredients"
    ],
    menuPhotos: [
      "https://images.unsplash.com/photo-1568254183919-78a4f43a2877?w=800",
      "https://images.unsplash.com/photo-1495147466023-ac5c588e2e94?w=800"
    ],
    staffKnowledgeScore: 5,
    celiacSafetyScore: 10
  },
  {
    name: "Off the Gluten Path - Woodstock",
    slug: "off-the-gluten-path-woodstock",
    address: "7 Ravenscraig Road, Woodstock, Cape Town, 8060",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Sat: 8:00AM – 5:00PM",
    phone: "+27 21 447 7741",
    website: "www.offtheglutenpath.co.za",
    directionsUrl: "https://www.google.com/maps/search/Off+the+Gluten+Path+Woodstock+Cape+Town",
    specialty: "100% Gluten-Free Bakery",
    overview: "The Woodstock branch of Off the Gluten Path offers the same exceptional quality and 100% gluten-free experience as the Sea Point location. Located in the trendy Woodstock area, this bakery is perfect for picking up fresh bread and treats.",
    menuHighlights: ["🥐 Fresh Pastries (GF)", "🥖 Breads (GF)", "🍰 Cakes (GF)", "🥗 Light Meals (GF)"],
    proTip: "Great for picking up bread for the week!",
    icon: "🥐",
    featured: false,
    cuisineTypes: ["Bakery", "Café"],
    celiacSafe: "dedicated-facility",
    menuType: "fully-gluten-free",
    rating: 4.8,
    reviewCount: 156,
    lat: -33.9269,
    lng: 18.4439,
    venueType: "bakery",
    photos: [
      "https://images.unsplash.com/photo-1509440159596-0249088772ff?w=800",
      "https://images.unsplash.com/photo-1486427944299-d1955d23e34d?w=800"
    ],
    menuNotes: [
      "Same menu as Sea Point location",
      "Dedicated gluten-free facility",
      "Weekly bread subscriptions available"
    ],
    menuPhotos: [
      "https://images.unsplash.com/photo-1568254183919-78a4f43a2877?w=800"
    ],
    staffKnowledgeScore: 5,
    celiacSafetyScore: 10
  },
  {
    name: "Tashas Waterfront",
    slug: "tashas-waterfront",
    address: "Shop 7117, Victoria & Alfred Waterfront, Cape Town, 8001",
    city: "Cape Town",
    country: "South Africa",
    hours: "Daily: 7:00AM – 10:00PM",
    phone: "+27 21 419 0011",
    website: "www.tashas.co.za",
    directionsUrl: "https://www.google.com/maps/search/Tashas+Waterfront+Cape+Town",
    specialty: "Upscale Café with GF Options",
    overview: "Tashas is an upscale café chain known for its stylish ambiance and extensive menu with clearly marked gluten-free options. The staff are well-trained in handling celiac requirements and can guide you through safe choices.",
    menuHighlights: ["🥗 Salads with GF Options", "🍳 All-Day Breakfast (GF options)", "🥩 Grilled Proteins (GF)", "🍰 GF Desserts Available"],
    proTip: "Ask for the gluten-free menu - staff are well-trained!",
    icon: "🍽️",
    featured: true,
    cuisineTypes: ["Café", "International", "Healthy"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.5,
    reviewCount: 312,
    lat: -33.9036,
    lng: 18.4214,
    venueType: "cafe",
    photos: [
      "https://images.unsplash.com/photo-1554118811-1e0d58224f24?w=800",
      "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=800",
      "https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=800"
    ],
    menuNotes: [
      "Dedicated GF menu available on request",
      "Kitchen uses separate prep areas for GF orders",
      "Staff trained on cross-contamination prevention",
      "Gluten-free bread available as substitution"
    ]
  },
  {
    name: "Hacienda Coastal Mexican",
    slug: "hacienda-coastal-mexican",
    address: "92 Bree St, Cape Town City Centre, Cape Town, 8000",
    city: "Cape Town",
    country: "South Africa",
    hours: "Daily: 12:00PM – 10:00PM",
    phone: "+27 21 422 2266",
    website: "www.hacienda.co.za",
    directionsUrl: "https://www.google.com/maps/search/Hacienda+Coastal+Mexican+92+Bree+St+Cape+Town",
    specialty: "Mexican Cuisine with GF Options",
    overview: "Hacienda offers authentic Mexican coastal cuisine with many naturally gluten-free options. Their corn-based dishes and fresh ingredients make it a great choice for celiacs who love Mexican flavors.",
    menuHighlights: ["🌮 Corn Tortilla Tacos (GF)", "🥑 Fresh Guacamole (GF)", "🍲 Mexican Rice Bowls (GF)", "🌶️ Fresh Salsas (GF)"],
    proTip: "Corn tortillas are naturally gluten-free - perfect for tacos!",
    icon: "🌮",
    featured: true,
    cuisineTypes: ["Mexican", "Coastal", "Latin"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.4,
    reviewCount: 189,
    lat: -33.9208,
    lng: 18.4172,
    venueType: "restaurant",
    photos: [
      "https://images.unsplash.com/photo-1565299585323-38d6b0865b47?w=800",
      "https://images.unsplash.com/photo-1551504734-5ee1c4a1479b?w=800",
      "https://images.unsplash.com/photo-1599974579688-8dbdd335c77f?w=800"
    ],
    menuNotes: [
      "Corn tortillas used for all GF tacos",
      "Specify celiac when ordering",
      "Many naturally GF options available",
      "Be careful with marinated meats - ask about flour"
    ]
  },
  {
    name: "GOLD Restaurant",
    slug: "gold-restaurant",
    address: "15 Bennett St, Green Point, Cape Town, 8005",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Sat: 7:00PM – 11:00PM",
    phone: "+27 21 421 4653",
    website: "www.goldrestaurant.co.za",
    directionsUrl: "https://www.google.com/maps/search/GOLD+Restaurant+15+Bennett+St+Green+Point+Cape+Town",
    specialty: "African Dining Experience",
    overview: "GOLD Restaurant offers a unique African dining experience with a 14-course taste safari. They accommodate dietary requirements including gluten-free with advance notice, creating a special experience for celiacs.",
    menuHighlights: ["🍛 African Tasting Menu", "🍲 Traditional Stews (GF options)", "🎵 Live African Entertainment", "🌍 Pan-African Cuisine"],
    proTip: "Book in advance and mention gluten-free requirements!",
    icon: "🌍",
    featured: true,
    cuisineTypes: ["African", "Fine Dining", "Cultural"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.6,
    reviewCount: 267,
    lat: -33.9101,
    lng: 18.4077,
    venueType: "restaurant",
    photos: [
      "https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=800",
      "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=800"
    ],
    menuNotes: [
      "Call ahead to arrange GF menu",
      "Many traditional African dishes are naturally GF",
      "Includes live entertainment and cultural experience",
      "Modified tasting menu available for celiacs"
    ]
  },
  {
    name: "Wildsprout - Constantia",
    slug: "wildsprout-constantia",
    address: "Shop 1, Constantia Village Courtyard, Constantia, Cape Town, 7708",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Fri: 7:00AM – 4:00PM, Sat–Sun: 8:00AM – 3:00PM",
    phone: "+27 21 794 7217",
    website: "www.wildsprout.co.za",
    directionsUrl: "https://www.google.com/maps/search/Wildsprout+Constantia+Village+Cape+Town",
    specialty: "Health-Focused Café",
    overview: "Wildsprout is a health-focused café offering organic, wholesome food with extensive gluten-free options. Perfect for health-conscious diners looking for clean, nutritious meals.",
    menuHighlights: ["🥗 Super Salads (GF)", "🥤 Fresh Smoothies & Juices", "🍳 Healthy Breakfasts (GF options)", "🥙 GF Wraps & Bowls"],
    proTip: "Their acai bowls are a must-try!",
    icon: "🥗",
    featured: true,
    cuisineTypes: ["Healthy", "Organic", "Café"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.7,
    reviewCount: 198,
    lat: -34.0191,
    lng: 18.4321,
    venueType: "cafe",
    photos: [
      "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=800",
      "https://images.unsplash.com/photo-1490645935967-10de6ba17061?w=800"
    ],
    menuNotes: [
      "Many menu items naturally GF",
      "GF bread available for sandwiches",
      "Organic ingredients prioritized",
      "Clear allergen labeling on menu"
    ]
  },
  {
    name: "Zenzero",
    slug: "zenzero",
    address: "Shop 2A, The Promenade, Victoria Rd, Camps Bay, Cape Town, 8040",
    city: "Cape Town",
    country: "South Africa",
    hours: "Daily: 12:00PM – 10:00PM",
    phone: "+27 21 438 0983",
    website: "www.zenzero.co.za",
    directionsUrl: "https://www.google.com/maps/search/Zenzero+Camps+Bay+Cape+Town",
    specialty: "Italian Restaurant with GF Options",
    overview: "Zenzero in Camps Bay offers Italian cuisine with stunning ocean views. They offer gluten-free pasta and pizza options for those who love Italian food but need to avoid gluten.",
    menuHighlights: ["🍝 GF Pasta Options", "🍕 GF Pizza Available", "🥗 Fresh Salads", "🍰 GF Desserts"],
    proTip: "Request GF pasta when ordering!",
    icon: "🍝",
    featured: true,
    cuisineTypes: ["Italian", "Mediterranean"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.3,
    reviewCount: 234,
    lat: -33.9506,
    lng: 18.3778,
    venueType: "restaurant",
    photos: [
      "https://images.unsplash.com/photo-1481931098730-318b6f776db0?w=800",
      "https://images.unsplash.com/photo-1565299624946-b28f40a0ae38?w=800"
    ],
    menuNotes: [
      "GF pasta and pizza crusts available",
      "Inform server about celiac disease",
      "Beautiful ocean views from terrace",
      "Shared kitchen - discuss cross-contamination"
    ]
  },
  {
    name: "Codfather Seafood & Sushi",
    slug: "codfather-seafood-sushi",
    address: "37 The Dr, Camps Bay, Cape Town, 8040",
    city: "Cape Town",
    country: "South Africa",
    hours: "Daily: 12:00PM – 10:30PM",
    phone: "+27 21 438 0782",
    website: "www.codfather.co.za",
    directionsUrl: "https://www.google.com/maps/search/Codfather+Seafood+Sushi+Camps+Bay+Cape+Town",
    specialty: "Fresh Seafood & Sushi",
    overview: "Codfather is famous for its unique concept where you choose your fresh seafood from the market display. Many options are naturally gluten-free, making it a great choice for seafood lovers with celiac.",
    menuHighlights: ["🦐 Fresh Seafood (GF)", "🍣 Sushi (ask for GF soy sauce)", "🦞 Lobster & Prawns (GF)", "🐟 Grilled Fish (GF)"],
    proTip: "Choose your own fish and they'll cook it to your preference!",
    icon: "🦐",
    featured: true,
    cuisineTypes: ["Seafood", "Sushi", "Japanese"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.6,
    reviewCount: 345,
    lat: -33.9499,
    lng: 18.3773,
    venueType: "restaurant",
    photos: [
      "https://images.unsplash.com/photo-1579631542720-3a87824fff86?w=800",
      "https://images.unsplash.com/photo-1534604973900-c43ab4c2e0ab?w=800"
    ],
    menuNotes: [
      "Fresh seafood is naturally GF",
      "Bring your own GF soy sauce for sushi",
      "Grilled options are safest",
      "Avoid battered/fried items"
    ]
  },
  {
    name: "Time Out Market",
    slug: "time-out-market",
    address: "The Old Power Station, Dock Rd, Victoria & Alfred Waterfront, Cape Town, 7806",
    city: "Cape Town",
    country: "South Africa",
    hours: "Daily: 9:00AM – 11:00PM",
    phone: "+27 21 418 8685",
    website: "www.timeoutmarket.com/capetown",
    directionsUrl: "https://www.google.com/maps/search/Time+Out+Market+Cape+Town+Waterfront",
    specialty: "Food Hall with Multiple Vendors",
    overview: "Time Out Market features multiple vendors, many offering gluten-free options. Great for groups with different dietary needs as everyone can find something they like.",
    menuHighlights: ["🍔 Multiple Cuisine Options", "🌮 Various GF-Friendly Stalls", "🍰 Desserts (some GF)", "🍷 Wine & Drinks"],
    proTip: "Check with each vendor for GF options!",
    icon: "🏪",
    featured: true,
    cuisineTypes: ["Food Hall", "Various"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.4,
    reviewCount: 456,
    lat: -33.9051,
    lng: 18.4206,
    venueType: "restaurant",
    photos: [
      "https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=800",
      "https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=800"
    ],
    menuNotes: [
      "Multiple vendors - check each for GF options",
      "Dedicated GF stalls available",
      "Great for groups with mixed dietary needs",
      "Cross-contamination risk varies by vendor"
    ]
  },
  {
    name: "Kanéla Café",
    slug: "kanela-cafe",
    address: "78 Regent Rd, Sea Point, Cape Town, 8001",
    city: "Cape Town",
    country: "South Africa",
    hours: "Daily: 7:00AM – 4:00PM",
    phone: "+27 21 434 1128",
    website: "www.kanela.co.za",
    directionsUrl: "https://www.google.com/maps/search/Kanela+Cafe+78+Regent+Rd+Sea+Point+Cape+Town",
    specialty: "Greek-Mediterranean Café",
    overview: "Kanéla offers Greek-Mediterranean cuisine with many naturally gluten-free options in a relaxed seaside setting. Their mezze platters and grilled dishes are excellent choices for celiacs.",
    menuHighlights: ["🥗 Greek Salads (GF)", "🍳 Mediterranean Breakfast (GF options)", "🍖 Grilled Meats (GF)", "🧀 Feta & Mezze Platters (GF)"],
    proTip: "Their mezze platter is perfect for sharing!",
    icon: "🥗",
    featured: false,
    cuisineTypes: ["Greek", "Mediterranean", "Café"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.4,
    reviewCount: 156,
    lat: -33.9188,
    lng: 18.3862,
    venueType: "cafe",
    photos: [
      "https://images.unsplash.com/photo-1544025162-d76694265947?w=800",
      "https://images.unsplash.com/photo-1540189549336-e6e99c3679fe?w=800"
    ],
    menuNotes: [
      "Greek cuisine naturally has many GF options",
      "Avoid pita bread and pastries",
      "Grilled meats and seafood are safe",
      "Ask about dressings and marinades"
    ]
  },
  {
    name: "Off the Gluten Path - Woodstock Store",
    slug: "off-the-gluten-path-woodstock-store",
    address: "7 Ravenscraig Road, Woodstock, Cape Town, 8060",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Sat: 8:00AM – 5:00PM",
    phone: "+27 21 447 7741",
    website: "www.offtheglutenpath.co.za",
    directionsUrl: "https://www.google.com/maps/search/Off+the+Gluten+Path+Woodstock+Cape+Town",
    specialty: "Gluten-Free Products & Bakery",
    overview: "Off the Gluten Path's Woodstock location offers a wide range of gluten-free products including fresh baked goods, pantry staples, and specialty items. A one-stop shop for celiacs.",
    menuHighlights: ["🥖 Fresh GF Breads", "🍪 GF Baked Goods", "🛒 GF Pantry Staples", "🧁 GF Treats"],
    proTip: "Stock up on bread and treats for the week!",
    icon: "🛒",
    featured: false,
    cuisineTypes: ["Grocery", "Bakery", "Gluten-Free"],
    celiacSafe: "dedicated-facility",
    menuType: "fully-gluten-free",
    rating: 4.8,
    reviewCount: 89,
    lat: -33.9269,
    lng: 18.4439,
    venueType: "gf-products",
    photos: ["https://images.unsplash.com/photo-1604719312566-8912e9227c6a?w=800"]
  },
  {
    name: "Wildsprout - Constantia",
    slug: "wildsprout-constantia-store",
    address: "Shop 1, Constantia Village Courtyard, Constantia, Cape Town, 7708",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Fri: 7:00AM – 4:00PM, Sat–Sun: 8:00AM – 3:00PM",
    phone: "+27 21 794 7217",
    website: "www.wildsprout.co.za",
    directionsUrl: "https://www.google.com/maps/search/Wildsprout+Constantia+Village+Cape+Town",
    specialty: "Organic Health Store",
    overview: "Wildsprout offers a curated selection of organic, health-focused products with many gluten-free options. Great for finding specialty GF items and fresh organic produce.",
    menuHighlights: ["🥗 Organic Produce", "🍞 GF Breads", "🥜 Health Foods", "🧃 Fresh Juices"],
    proTip: "Check their fridge section for fresh GF meals!",
    icon: "🌿",
    featured: false,
    cuisineTypes: ["Organic", "Health Store", "Gluten-Free"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.6,
    reviewCount: 112,
    lat: -34.0191,
    lng: 18.4321,
    venueType: "supermarket",
    photos: ["https://images.unsplash.com/photo-1542838132-92c53300491e?w=800"]
  },
  {
    name: "Real Food Co",
    slug: "real-food-co-somerset-west",
    address: "43 Panorama Dr, Somerset West, Cape Town, 7130",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Fri: 8:00AM – 6:00PM, Sat: 8:00AM – 4:00PM",
    phone: "+27 21 851 4044",
    website: "www.realfoodco.co.za",
    directionsUrl: "https://www.google.com/maps/search/Real+Food+Co+Somerset+West+Cape+Town",
    specialty: "Natural & Organic Grocery",
    overview: "Real Food Co specializes in natural, organic, and free-from products. Their extensive gluten-free section includes baking supplies, snacks, and frozen meals.",
    menuHighlights: ["🌾 GF Baking Supplies", "🍫 GF Snacks", "❄️ Frozen GF Meals", "🥛 Dairy Alternatives"],
    proTip: "Great selection of GF baking flours!",
    icon: "🏪",
    featured: false,
    cuisineTypes: ["Organic", "Health Store", "Natural Foods"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.5,
    reviewCount: 78,
    lat: -34.0825,
    lng: 18.8441,
    venueType: "supermarket",
    photos: ["https://images.unsplash.com/photo-1578916171728-46686eac8d58?w=800"]
  },
  {
    name: "Sea Point Spar",
    slug: "sea-point-spar",
    address: "94 Regent Rd, Sea Point, Cape Town, 8005",
    city: "Cape Town",
    country: "South Africa",
    hours: "Daily: 7:00AM – 9:00PM",
    phone: "+27 21 434 1099",
    website: "www.spar.co.za",
    directionsUrl: "https://www.google.com/maps/search/Sea+Point+Spar+94+Regent+Rd+Cape+Town",
    specialty: "Supermarket with GF Section",
    overview: "Sea Point Spar has a dedicated gluten-free section with a good variety of GF products including breads, pastas, snacks, and frozen items. Convenient location for Sea Point residents.",
    menuHighlights: ["🍞 GF Breads", "🍝 GF Pasta", "🍕 Frozen GF Pizza", "🍪 GF Snacks"],
    proTip: "Check the freezer section for GF pizza bases!",
    icon: "🛒",
    featured: false,
    cuisineTypes: ["Supermarket", "Grocery"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.2,
    reviewCount: 156,
    lat: -33.9188,
    lng: 18.3862,
    venueType: "supermarket",
    photos: ["https://images.unsplash.com/photo-1604719312566-8912e9227c6a?w=800"]
  },
  {
    name: "Woolworths Food Village Square",
    slug: "woolworths-food-village-square",
    address: "The Village Square, C/O Oxford & Queen St, Durbanville, Cape Town, 7550",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Fri: 8:00AM – 7:00PM, Sat–Sun: 8:00AM – 5:00PM",
    phone: "+27 21 975 0500",
    website: "www.woolworths.co.za",
    directionsUrl: "https://www.google.com/maps/search/Woolworths+Food+Village+Square+Durbanville+Cape+Town",
    specialty: "Premium Supermarket with GF Range",
    overview: "Woolworths is known for its quality food products and has an excellent gluten-free range. Their 'Free From' section offers certified GF products including fresh breads, meals, and snacks.",
    menuHighlights: ["🥖 Fresh GF Breads", "🥗 Ready-Made GF Meals", "🍰 GF Desserts", "🧀 GF Snacks"],
    proTip: "Their in-store bakery has fresh GF bread daily!",
    icon: "🛍️",
    featured: false,
    cuisineTypes: ["Premium Supermarket", "Grocery"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.4,
    reviewCount: 234,
    lat: -33.8346,
    lng: 18.6463,
    venueType: "supermarket",
    photos: ["https://images.unsplash.com/photo-1534723452862-4c874018d66d?w=800"]
  },
  {
    name: "SPAR De Waterkant",
    slug: "spar-de-waterkant",
    address: "27 Somerset Rd, De Waterkant, Cape Town, 8005",
    city: "Cape Town",
    country: "South Africa",
    hours: "Daily: 7:00AM – 10:00PM",
    phone: "+27 21 421 1963",
    website: "www.spar.co.za",
    directionsUrl: "https://www.google.com/maps/search/SPAR+27+Somerset+Rd+De+Waterkant+Cape+Town",
    specialty: "City Centre Supermarket",
    overview: "Conveniently located in De Waterkant, this SPAR offers a selection of gluten-free products. Great for visitors staying in the city centre looking for GF essentials.",
    menuHighlights: ["🍞 GF Bread Options", "🥫 GF Canned Goods", "🍫 GF Snacks", "🥛 Dairy Alternatives"],
    proTip: "Open late for emergency GF shopping!",
    icon: "🏪",
    featured: false,
    cuisineTypes: ["Supermarket", "Convenience"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.0,
    reviewCount: 89,
    lat: -33.9164,
    lng: 18.4147,
    venueType: "supermarket",
    photos: ["https://images.unsplash.com/photo-1604719312566-8912e9227c6a?w=800"]
  },
  {
    name: "The Village Hub",
    slug: "the-village-hub-scarborough",
    address: "2 Watsonia Ln, Scarborough, Cape Town, 7975",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Sat: 8:00AM – 5:00PM, Sun: 9:00AM – 2:00PM",
    phone: "+27 21 780 1452",
    website: "",
    directionsUrl: "https://www.google.com/maps/search/The+Village+Hub+Scarborough+Cape+Town",
    specialty: "Community Store with GF Options",
    overview: "A charming community store in Scarborough village offering local and artisan products including a selection of gluten-free items. Perfect for visitors exploring the southern peninsula.",
    menuHighlights: ["🥖 Local GF Breads", "🍯 Artisan Products", "☕ Coffee & Tea", "🧁 GF Treats"],
    proTip: "Support local – great artisan GF products!",
    icon: "🏘️",
    featured: false,
    cuisineTypes: ["Community Store", "Artisan"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.3,
    reviewCount: 45,
    lat: -34.2089,
    lng: 18.3717,
    venueType: "supermarket",
    photos: ["https://images.unsplash.com/photo-1556742049-0cfed4f6a45d?w=800"]
  },
  {
    name: "Wildsprout - Sea Point",
    slug: "wildsprout-sea-point",
    address: "267 Main Rd, Sea Point, Cape Town, 8005",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Fri: 7:00AM – 5:00PM, Sat–Sun: 8:00AM – 4:00PM",
    phone: "+27 21 433 2066",
    website: "www.wildsprout.co.za",
    directionsUrl: "https://www.google.com/maps/search/Wildsprout+267+Main+Rd+Sea+Point+Cape+Town",
    specialty: "Organic Health Store & Café",
    overview: "The Sea Point Wildsprout location offers organic groceries, health foods, and a café with gluten-free options. Great for stocking up on GF products while enjoying a healthy meal.",
    menuHighlights: ["🌿 Organic Groceries", "🥗 Fresh GF Meals", "🍞 GF Bakery Items", "🥤 Fresh Smoothies"],
    proTip: "Combine grocery shopping with a healthy GF lunch!",
    icon: "🌿",
    featured: false,
    cuisineTypes: ["Organic", "Health Store", "Café"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.7,
    reviewCount: 167,
    lat: -33.9175,
    lng: 18.3856,
    venueType: "supermarket",
    photos: ["https://images.unsplash.com/photo-1542838132-92c53300491e?w=800"]
  },
  {
    name: "Pasta Regalo",
    slug: "pasta-regalo",
    address: "Unit A7, Hyde Park, Ottery, Cape Town, South Africa",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Fri: 8:00AM – 5:00PM",
    phone: "+27 21 703 7304",
    website: "www.pastaregalo.co.za",
    directionsUrl: "https://www.google.com/maps/search/Pasta+Regalo+Hyde+Park+Ottery+Cape+Town",
    specialty: "Gluten-Free Pasta Products",
    overview: "Pasta Regalo produces a range of gluten-free pasta made from maize — egg-free, wheat-free, and GMO-free. Their products contain no preservatives or artificial flavours. Based in Ottery, Cape Town, they offer varieties like Beetroot Fettuccine, Spinach Tagliatelle, and classic pasta shapes, all certified gluten-free.",
    menuHighlights: ["🍝 GF Beetroot Fettuccine", "🍝 GF Spinach Tagliatelle", "🍝 Classic GF Pasta Shapes", "🌽 Maize-Based, Egg-Free"],
    proTip: "Cook for 8–12 minutes in salted boiling water until al dente. 120g dry pasta = one serving.",
    icon: "🍝",
    featured: true,
    cuisineTypes: ["Pasta", "Gluten-Free Products"],
    celiacSafe: "dedicated-facility",
    menuType: "fully-gluten-free",
    rating: 4.7,
    reviewCount: 42,
    lat: -34.0048,
    lng: 18.5125,
    venueType: "gf-products",
    photos: [pastaRegaloPhoto],
    menuNotes: [
      "All pasta made from maize — no wheat, no egg",
      "GMO-free ingredients",
      "No preservatives or artificial flavours",
      "Allergens: None listed on packaging"
    ],
    staffKnowledgeScore: 5,
    celiacSafetyScore: 10
  },
  {
    name: "Sir Eat-A-Lot",
    slug: "sir-eat-a-lot",
    address: "Cape Town, South Africa",
    city: "Cape Town",
    country: "South Africa",
    hours: "Contact for hours",
    phone: "",
    website: "",
    directionsUrl: "https://www.google.com/maps/search/Sir+Eat+A+Lot+Gluten+Free+Cape+Town",
    specialty: "Gluten-Free Bread & Baked Goods",
    overview: "Sir Eat-A-Lot is a dedicated gluten-free bakery handcrafting certified gluten-free bread in Cape Town. Their white bread is dairy-free, trans-fat free, cholesterol-free, and suitable for vegetarians. Free from wheat, soya, and dairy — a safe choice for celiacs.",
    menuHighlights: ["🍞 GF White Bread", "🍞 GF Brown Bread", "🥖 Certified Gluten-Free", "🌱 Dairy-Free & Vegetarian"],
    proTip: "Their bread is a source of energy and dietary fibre — great as a daily staple!",
    icon: "🍞",
    featured: true,
    cuisineTypes: ["Bakery", "Bread", "Gluten-Free Products"],
    celiacSafe: "dedicated-facility",
    menuType: "fully-gluten-free",
    rating: 4.6,
    reviewCount: 35,
    lat: -34.0000,
    lng: 18.4800,
    venueType: "gf-products",
    photos: [sirEatALotPhoto],
    menuNotes: [
      "Handcrafted in a dedicated gluten-free facility",
      "Certified gluten-free",
      "Free from wheat, soya, and dairy",
      "Dairy-free, trans-fat free, cholesterol-free",
      "Suitable for vegetarians"
    ],
    staffKnowledgeScore: 5,
    celiacSafetyScore: 10
  },
  {
    name: "Oranjezicht City Farm Market",
    slug: "oranjezicht-city-farm-market",
    address: "SIDD Kitchen, Granger Bay Blvd, V&A Waterfront, Cape Town, 8001",
    city: "Cape Town",
    country: "South Africa",
    hours: "Sat–Sun: 8:00AM – 2:00PM",
    phone: "",
    website: "www.ozcf.co.za",
    directionsUrl: "https://www.google.com/maps/search/Oranjezicht+City+Farm+Market+Cape+Town",
    specialty: "Organic Farmers Market",
    overview: "Oranjezicht City Farm Market is Cape Town's beloved Saturday and Sunday farmers market at the Waterfront. Many vendors offer naturally gluten-free produce, baked goods, and street food. It's a great spot to wander, sample, and find fresh celiac-friendly bites from local artisans.",
    menuHighlights: ["🥕 Fresh Organic Produce", "🍞 Artisan GF Baked Goods", "🥗 Ready-to-Eat GF Meals", "🥤 Fresh Juices & Smoothies"],
    proTip: "Arrive early for the best selection of fresh GF baked goods!",
    icon: "🥬",
    featured: true,
    cuisineTypes: ["Market", "Organic", "Street Food"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.7,
    reviewCount: 312,
    lat: -33.9030,
    lng: 18.4200,
    venueType: "street-food",
    photos: [
      "https://images.unsplash.com/photo-1488459716781-31db52582fe9?w=800",
      "https://images.unsplash.com/photo-1542838132-92c53300491e?w=800"
    ],
    menuNotes: [
      "Multiple vendors - ask each about GF options",
      "Fresh organic produce is naturally GF",
      "Some stalls dedicated GF - look for signage",
      "Great for discovering new local GF producers"
    ]
  },
  {
    name: "The Neighbourgoods Market",
    slug: "the-neighbourgoods-market",
    address: "The Old Biscuit Mill, 375 Albert Rd, Woodstock, Cape Town, 7915",
    city: "Cape Town",
    country: "South Africa",
    hours: "Sat: 9:00AM – 3:00PM",
    phone: "",
    website: "www.neighbourgoodsmarket.co.za",
    directionsUrl: "https://www.google.com/maps/search/Neighbourgoods+Market+Old+Biscuit+Mill+Cape+Town",
    specialty: "Trendy Weekend Food Market",
    overview: "The Neighbourgoods Market at The Old Biscuit Mill is Cape Town's iconic Saturday market. A vibrant mix of street food vendors, artisans, and local producers. Many stalls offer gluten-free options from gourmet burgers to fresh juices and baked treats.",
    menuHighlights: ["🍔 Gourmet GF Street Burgers", "🥗 Fresh Salads & Bowls", "🧁 Artisan GF Cupcakes", "🍹 Craft Drinks & Juices"],
    proTip: "Check the upstairs deck for more seating and a great vibe!",
    icon: "🎪",
    featured: true,
    cuisineTypes: ["Market", "Street Food", "Various"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.6,
    reviewCount: 456,
    lat: -33.9275,
    lng: 18.4570,
    venueType: "street-food",
    photos: [
      "https://images.unsplash.com/photo-1555396273-367ea4eb4db5?w=800",
      "https://images.unsplash.com/photo-1498837167922-ddd27525d352?w=800"
    ],
    menuNotes: [
      "Over 100 traders - many with GF options",
      "Ask vendors directly about ingredients",
      "Busy market - arrive before 11am to beat queues",
      "Some dedicated GF stalls available"
    ]
  },
  {
    name: "Mojo Market",
    slug: "mojo-market",
    address: "30 Regent Rd, Sea Point, Cape Town, 8005",
    city: "Cape Town",
    country: "South Africa",
    hours: "Daily: 10:00AM – 10:00PM",
    phone: "+27 21 433 0866",
    website: "www.mojomarket.co.za",
    directionsUrl: "https://www.google.com/maps/search/Mojo+Market+30+Regent+Rd+Sea+Point+Cape+Town",
    specialty: "Indoor Food Hall & Market",
    overview: "Mojo Market is a bustling indoor food hall in Sea Point with over 40 food and drink stalls. You'll find everything from Asian fusion to health bowls, with several vendors offering clearly marked gluten-free street food options in a lively, covered setting.",
    menuHighlights: ["🍜 Asian Fusion Bowls (GF options)", "🥙 Health Wraps & Bowls", "🍣 Sushi & Poke (GF)", "🍰 GF Desserts"],
    proTip: "The health bowl vendors usually have the best GF options!",
    icon: "🍜",
    featured: true,
    cuisineTypes: ["Food Hall", "Asian Fusion", "Street Food"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.5,
    reviewCount: 378,
    lat: -33.9180,
    lng: 18.3860,
    venueType: "street-food",
    photos: [
      "https://images.unsplash.com/photo-1517248135467-4c7edcad34c4?w=800",
      "https://images.unsplash.com/photo-1552566626-52f8b828add9?w=800"
    ],
    menuNotes: [
      "Over 40 vendors under one roof",
      "Several vendors clearly mark GF items",
      "Live music on weekends",
      "Cross-contamination risk varies by stall"
    ]
  },
  {
    name: "Vegan Goods Market",
    slug: "vegan-goods-market",
    address: "Various Locations, Cape Town",
    city: "Cape Town",
    country: "South Africa",
    hours: "Check website for upcoming dates",
    phone: "",
    website: "www.veangoodsamarket.com",
    directionsUrl: "https://www.google.com/maps/search/Vegan+Goods+Market+Cape+Town",
    specialty: "Plant-Based & Vegan Market",
    overview: "Vegan Goods Market is a pop-up market celebrating plant-based living. Many of the vendors naturally offer gluten-free options alongside their vegan creations, from raw desserts to savoury street food. A welcoming space for anyone exploring GF and plant-based food.",
    menuHighlights: ["🥗 Raw GF Desserts", "🌮 Plant-Based Tacos (GF)", "🥪 GF & Vegan Sandwiches", "🥤 Smoothie Bowls"],
    proTip: "Most vegan baked goods are also GF - just ask to confirm!",
    icon: "🌱",
    featured: false,
    cuisineTypes: ["Vegan", "Plant-Based", "Market"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.4,
    reviewCount: 156,
    lat: -33.9249,
    lng: 18.4241,
    venueType: "street-food",
    photos: [
      "https://images.unsplash.com/photo-1490645935967-10de6ba17061?w=800",
      "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=800"
    ],
    menuNotes: [
      "Pop-up market - check dates before visiting",
      "Many vendors offer both vegan and GF",
      "Great for trying innovative plant-based GF foods",
      "Friendly vendors happy to discuss ingredients"
    ]
  },
  {
    name: "Yen's Vietnamese Street Food",
    slug: "yens-vietnamese-street-food",
    address: "Various Pop-Up Locations, Cape Town",
    city: "Cape Town",
    country: "South Africa",
    hours: "Check social media for schedule",
    phone: "",
    website: "",
    directionsUrl: "https://www.google.com/maps/search/Yens+Vietnamese+Street+Food+Cape+Town",
    specialty: "Authentic Vietnamese Street Food",
    overview: "Yen's Vietnamese Street Food brings authentic flavours to Cape Town's streets through pop-up stalls and market appearances. Their rice noodle bowls and fresh spring rolls are naturally gluten-free, offering celiacs a delicious and safe taste of Vietnam.",
    menuHighlights: ["🍜 Pho with Rice Noodles (GF)", "🌯 Fresh Rice Paper Rolls (GF)", "🍲 Vietnamese Vermicelli Bowls (GF)", "🥢 Banh Mi (GF bread option)"],
    proTip: "Always double-check the soy sauce - ask for tamari!",
    icon: "🍜",
    featured: true,
    cuisineTypes: ["Vietnamese", "Asian", "Street Food"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.5,
    reviewCount: 89,
    lat: -33.9200,
    lng: 18.4150,
    venueType: "street-food",
    photos: [
      "https://images.unsplash.com/photo-1582878826629-29b7ad1cdc43?w=800",
      "https://images.unsplash.com/photo-1555126634-323283e090fa?w=800"
    ],
    menuNotes: [
      "Rice noodles are naturally GF",
      "Ask for tamari instead of regular soy sauce",
      "Fresh rolls are usually safe",
      "Pop-up locations vary - follow on social media"
    ]
  },
  {
    name: "Wild Eatery",
    slug: "wild-eatery",
    address: "Kloof St, Gardens, Cape Town, 8001",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Sun: 8:00AM – 4:00PM",
    phone: "",
    website: "www.wildeatery.co.za",
    directionsUrl: "https://www.google.com/maps/search/Wild+Eatery+Kloof+Street+Cape+Town",
    specialty: "Health-Focused Street Eatery",
    overview: "Wild Eatery is a trendy health-focused spot on Kloof Street serving vibrant bowls, wraps, and smoothies. They cater well to gluten-free diners with clearly marked options and a focus on fresh, whole ingredients that make celiac-friendly eating easy and delicious.",
    menuHighlights: ["🥗 Buddha Bowls (GF)", "🌯 GF Wraps", "🥤 Superfood Smoothies", "🥑 Avocado Toast (GF Bread)"],
    proTip: "Their buddha bowls are fully customisable and naturally GF!",
    icon: "🥑",
    featured: false,
    cuisineTypes: ["Healthy", "Café", "Street Food"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.3,
    reviewCount: 124,
    lat: -33.9250,
    lng: 18.4100,
    venueType: "street-food",
    photos: [
      "https://images.unsplash.com/photo-1546069901-ba9599a7e63c?w=800",
      "https://images.unsplash.com/photo-1512621776951-a57141f2eefd?w=800"
    ],
    menuNotes: [
      "GF options clearly marked on menu",
      "Focus on whole, unprocessed ingredients",
      "Great for quick healthy GF lunches",
      "Staff knowledgeable about allergens"
    ]
  },
  {
    name: "The Conscious Kitchen - Kloof Street",
    slug: "the-conscious-kitchen-kloof-street",
    address: "Kloof St, Gardens, Cape Town, 8001",
    city: "Cape Town",
    country: "South Africa",
    hours: "Mon–Sat: 7:00AM – 5:00PM, Sun: 8:00AM – 3:00PM",
    phone: "+27 21 424 4444",
    website: "www.theconsciouskitchen.co.za",
    directionsUrl: "https://www.google.com/maps/search/The+Conscious+Kitchen+Kloof+Street+Cape+Town",
    specialty: "Mindful Eating & GF Options",
    overview: "The Conscious Kitchen on Kloof Street is all about mindful, nourishing food. They offer a range of gluten-free dishes made with carefully sourced ingredients. Their menu is designed to accommodate various dietary needs without compromising on flavour.",
    menuHighlights: ["🥗 Nourishing Bowls (GF)", "🍳 GF Breakfast Options", "🧁 GF Baked Treats", "☕ Specialty Coffee"],
    proTip: "Their GF granola breakfast bowl is a local favourite!",
    icon: "🥗",
    featured: true,
    cuisineTypes: ["Healthy", "Café", "Gluten-Free Friendly"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.5,
    reviewCount: 198,
    lat: -33.9255,
    lng: 18.4105,
    venueType: "street-food",
    photos: [
      "https://images.unsplash.com/photo-1498837167922-46686eac8d58?w=800",
      "https://images.unsplash.com/photo-1504754524776-8f4f37790ca0?w=800"
    ],
    menuNotes: [
      "GF items clearly indicated on menu",
      "Uses separate prep areas for GF orders",
      "Organic and locally sourced ingredients",
      "Great coffee to pair with GF treats"
    ]
  },
  {
    name: "Mango Ginger",
    slug: "mango-ginger",
    address: "Various Market Locations, Cape Town",
    city: "Cape Town",
    country: "South Africa",
    hours: "Market hours vary",
    phone: "",
    website: "www.mangoginger.co.za",
    directionsUrl: "https://www.google.com/maps/search/Mango+Ginger+Cape+Town",
    specialty: "Asian-Inspired Street Food",
    overview: "Mango Ginger serves vibrant Asian-inspired street food at markets and pop-ups around Cape Town. Their menu features naturally gluten-free rice-based dishes, curries, and fresh salads that are full of flavour and safe for celiacs who love bold Asian tastes.",
    menuHighlights: ["🍛 Thai Curries (GF)", "🍚 Rice Noodle Salads (GF)", "🥗 Fresh Asian Slaw (GF)", "🍤 Rice Paper Rolls (GF)"],
    proTip: "Their green curry with jasmine rice is a must-try and naturally GF!",
    icon: "🍛",
    featured: false,
    cuisineTypes: ["Thai", "Asian", "Street Food"],
    celiacSafe: "protocols-in-place",
    menuType: "mixed-menu",
    rating: 4.4,
    reviewCount: 67,
    lat: -33.9230,
    lng: 18.4160,
    venueType: "street-food",
    photos: [
      "https://images.unsplash.com/photo-1455619452474-d2be8b1e970d?w=800",
      "https://images.unsplash.com/photo-1565557623262-b51c2513a641?w=800"
    ],
    menuNotes: [
      "Rice-based dishes are naturally GF",
      "Ask about soy sauce and marinades",
      "Fresh salads and curries are usually safe",
      "Follow their social media for market locations"
    ]
  }
];

export const getRestaurantBySlug = (slug: string): Restaurant | undefined => {
  return capeTownRestaurants.find(r => r.slug === slug);
};

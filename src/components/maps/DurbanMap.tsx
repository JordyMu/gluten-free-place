...
interface DurbanMapProps {
  restaurants: Restaurant[];
}

const DurbanMap = ({ restaurants }: DurbanMapProps) => {
  ...
  useEffect(() => {
    ...
    map.current = new mapboxgl.Map({
      ...
      center: [31.0218, -29.8587], // Durban center
      zoom: 11
    });
  ...
  useEffect(() => {
    ...
    restaurants.forEach((restaurant) => {
      ...
      el.className = 'flex items-center justify-center w-8 h-8 bg-blue-500 rounded-full text-white text-lg shadow-lg cursor-pointer hover:scale-110 transition-transform';
